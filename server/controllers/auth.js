...
exports.signin = (req, res) => {
  ...
  User.findOne({ email }).exec((err, user) => {
    ...
    // generate json web token to send to client ( user id, secret )
...
exports.forgotPassword = (req, res) => {
  ...
  User.findOne({ email }, (err, user) => {
    ...
    const emailData = {
      from: process.env.EMAIL_FROM,
      to: email,
      subjectt: `Password rest link`,
      html: `
        <h4>Your Daily Dump Password Reset</h4>
        <p>Please use the following link to rest your password:</p>
      ...
      function (err, decoded) {
        ...
        User.findOne({ resetPasswordLink }, (err, user) => {
          ...
          user.save((err, result) => {
            ...
            res.json({
              message: `Great! Now you can log in with yout new Password`,
            });
          });
        });
      }
